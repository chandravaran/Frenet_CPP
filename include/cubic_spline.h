 ...
 * @brief: This file contains definitions of a cubic spline class.
...
Vec_float vec_diff(Vec_float input){
  Vec_float output;
...
Vec_float cum_sum(Vec_float input){
  Vec_float output;
  float temp = 0;
...
public:
  ...
  Spline(Vec_float x_, Vec_float y_):x(x_), y(y_), nx(x_.size()), h(vec_diff(x_)), a(y_){
...
public:
  ...
  Spline2D(Vec_float x, Vec_float y){
    s = calc_s(x, y);
    sx = Spline(s, x);
    sy = Spline(s, y);
  };

  Vec_float set_s_x_y(Vec_float x, Vec_float y){
    s = calc_s(x, y);
    sx = Spline(s, x);
    sy = Spline(s, y);
  };

  Pos_float calc_postion(float s_t){
    float x = sx.calc(s_t);
    float y = sy.calc(s_t);
    return {{x, y}};
  };
...
private:
  Vec_float calc_s(Vec_float x, Vec_float y){
    Vec_float ds;
    Vec_float out_s{0};
    Vec_float dx = vec_diff(x);
    Vec_float dy = vec_diff(y);

    for(unsigned int i=0; i<dx.size(); i++){
      ds.push_back(std::sqrt(dx[i]*dx[i] + dy[i]*dy[i]));
    }

    Vec_float cum_ds = cum_sum(ds);
